...
export function WorkshopQuotationPrint({
  ...
  if (!ticket || !quote) {
    return (
      <div className="p-6">
        ...
        <Button className="mt-4" variant="outline" onClick={() => router.push(`/workshop?ticket=${ticketId}`)}>
          Volver a la orden
        </Button>
      </div>
    );
  ...
  const lines: QuoteLine[] = snapshotLines.length
    ...
    : ticket.lines.map((line) => ({
        ...
        billableQuantity: String(Math.max(0, Number(line.quantity) - Number(line.releasedQuantity))),
    ...
    <div className="mx-auto max-w-4xl space-y-5 p-4 sm:p-6 print:max-w-none print:p-0">
      <div className="flex flex-wrap items-start justify-between gap-3 print:hidden">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Lista para imprimir o guardar como archivo PDF.</p>
        </div>
      ...
      <article className="rounded-xl border border-zinc-200 bg-white p-6 text-zinc-950 shadow-sm sm:p-8 print:rounded-none print:border-0 print:p-0 print:shadow-none">
        <header className="flex items-start justify-between gap-6 border-b border-zinc-200 pb-5">
          <div>
            <p className="text-xs font-bold uppercase tracking-[0.18em] text-zinc-500">{brand.businessType}</p>
          ...
          <div className="text-right">
            ...
            <p className="mt-1 text-xl font-bold">{ticket.ticketNumber}-V{quote.version}</p>
            <Badge className="mt-2" variant="outline">Versión {quote.version}</Badge>
          </div>
        </header>
        ...
        <section className="grid gap-5 border-b border-zinc-200 py-5 sm:grid-cols-3">
          <div>
            ...
            <p className="text-sm text-zinc-600">{ticket.customer.email || 'Sin correo registrado'}</p>
          </div>
          <div>
          ...
          <div>
            ...
            {ticket.promisedAt ? <p className="text-sm text-zinc-600">Promesa: {formatDateTime(ticket.promisedAt)}</p> : null}
          </div>
        </section>
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {lines.map((line, index) => (
                <tr key={line.id ?? `${line.description}-${index}`} className="border-b border-zinc-100 align-top">
                  <td className="py-3 pr-3 font-medium">{line.description}</td>
                  <td className="py-3 pr-3 text-zinc-600">{lineTypeLabel[line.type]}</td>
                  <td className="py-3 text-right">{Number(line.billableQuantity ?? line.quantity).toLocaleString('es-DO')}</td>
                  <td className="py-3 text-right">{formatCurrency(Number(line.unitPrice))}</td>
                  <td className="py-3 pl-3 text-right font-semibold">{formatCurrency(Number(line.total))}</td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        <footer className="grid gap-6 border-t border-zinc-200 pt-5 sm:grid-cols-[1fr_18rem]">
          <div className="text-sm text-zinc-600">
            ...
            <p className="mt-1">{quote.note || ticket.customerNotes || ticket.diagnosis || 'Cotización sujeta a aprobación del cliente.'}</p>
          </div>
          <div className="space-y-2 text-sm">
            <div className="flex justify-between gap-5"><span>Mano de obra</span><strong>{formatCurrency(Number(quote.laborTotal))}</strong></div>
            <div className="flex justify-between gap-5"><span>Repuestos</span><strong>{formatCurrency(Number(quote.partsTotal))}</strong></div>
            {taxes > 0 ? <div className="flex justify-between gap-5"><span>Impuestos</span><strong>{formatCurrency(taxes)}</strong></div> : null}
            <div className="flex justify-between gap-5 border-t border-zinc-300 pt-3 text-lg"><span>Total</span><strong>{formatCurrency(Number(quote.total))}</strong></div>
          </div>
        </footer>

        <div className="mt-8 hidden items-center gap-2 border-t border-zinc-200 pt-4 text-xs text-zinc-500 print:flex">
          <Printer className="h-3.5 w-3.5" /> Documento generado desde {brand.name} · {formatDateTime(new Date())}
        </div>
      </article>
    </div>
  );
